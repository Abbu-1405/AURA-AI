...
import { CreativeDirectorReport, DesignStyle } from "./types";
...
export function getAiClient(): GoogleGenAI | null {
  ...
  if (apiKey && apiKey !== "MY_GEMINI_API_KEY") {
    try {
      ...
      console.log("Aura AI: Gemini SDK initialized successfully via lazy-loading.");
    } catch (error) {
...
export async function generateContentWithFallback(params: any, primaryModel: string = "gemini-3.5-flash"): Promise<any> {
  const modelsToTry = [
    primaryModel,
...
export function completeReport(raw: any, style: DesignStyle): CreativeDirectorReport {
 ...
 */
export function getPlaceholderReport(style: DesignStyle): CreativeDirectorReport {
  const reports: Record<DesignStyle, any> = {
    Minimal: {
      ...
      typography: {
        ...
        suggestedImprovements: "Introduce extreme contrast in weight (e.g., Thin vs Bold), widen the tracking on all uppercase subtitles, and expand line-spacing dramatically.",
...
export function sanitizeReport(parsed: any, style: DesignStyle): CreativeDirectorReport {
  ...
  const sanitized: any = {
    styleSelected: style,
    overallScore: parsed?.overallScore ? safeNum(parsed.overallScore, fallback.overallScore) : undefined,
    overallVerdict: safeString(parsed?.overallVerdict, fallback.overallVerdict),
    typography: {
      score: parsed?.typography?.score ? safeNum(parsed.typography.score, fallback.typography.score) : undefined,
      breakdown: parsed?.typography?.breakdown || fallback.typography.breakdown,
      breakdownExplanations: parsed?.typography?.breakdownExplanations || fallback.typography.breakdownExplanations,
      observation: safeString(parsed?.typography?.observation, fallback.typography.observation),
      problem: safeString(parsed?.typography?.problem, fallback.typography.problem),
      reason: safeString(parsed?.typography?.reason, fallback.typography.reason),
      solution: safeString(parsed?.typography?.solution, fallback.typography.solution),
    ...
    color: {
      score: parsed?.color?.score ? safeNum(parsed.color.score, fallback.color.score) : undefined,
      breakdown: parsed?.color?.breakdown || fallback.color.breakdown,
      breakdownExplanations: parsed?.color?.breakdownExplanations || fallback.color.breakdownExplanations,
      observation: safeString(parsed?.color?.observation, fallback.color.observation),
      problem: safeString(parsed?.color?.problem, fallback.color.problem),
      reason: safeString(parsed?.color?.reason, fallback.color.reason),
      solution: safeString(parsed?.color?.solution, fallback.color.solution),
      currentPalette: Array.isArray(parsed?.color?.currentPalette) ? parsed.color.currentPalette : fallback.color.currentPalette,
    ...
    sizePositioning: {
      score: parsed?.sizePositioning?.score ? safeNum(parsed.sizePositioning.score, fallback.sizePositioning.score) : undefined,
      breakdown: parsed?.sizePositioning?.breakdown || fallback.sizePositioning.breakdown,
      breakdownExplanations: parsed?.sizePositioning?.breakdownExplanations || fallback.sizePositioning.breakdownExplanations,
      observation: safeString(parsed?.sizePositioning?.observation, fallback.sizePositioning.observation),
      problem: safeString(parsed?.sizePositioning?.problem, fallback.sizePositioning.problem),
      reason: safeString(parsed?.sizePositioning?.reason, fallback.sizePositioning.reason),
      solution: safeString(parsed?.sizePositioning?.solution, fallback.sizePositioning.solution),
    ...
    background: {
      score: parsed?.background?.score ? safeNum(parsed.background.score, fallback.background.score) : undefined,
      breakdown: parsed?.background?.breakdown || fallback.background.breakdown,
      breakdownExplanations: parsed?.background?.breakdownExplanations || fallback.background.breakdownExplanations,
      observation: safeString(parsed?.background?.observation, fallback.background.observation),
      problem: safeString(parsed?.background?.problem, fallback.background.problem),
      reason: safeString(parsed?.background?.reason, fallback.background.reason),
      solution: safeString(parsed?.background?.solution, fallback.background.solution),
